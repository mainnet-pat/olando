...
export const investInIssuanceFund = async ({ investAmountBch, provider, address, privKey, wallet, councilMultisigContract, adminMultisigContract, olandoCategory, send = true, }) => {
    ...
    let investmentUtxo = userBchUtxos.find(u => u.satoshis >= BigInt(investAmountBch * 1e8) + 5000n);
    if (!investmentUtxo) {
        await wallet.send([new SendRequest({
                cashaddr: wallet.cashaddr,
                unit: 'sat',
        ...
        userUtxos = await provider.getUtxos(address);
        userBchUtxos = userUtxos.filter(u => u.token === undefined);
        investmentUtxo = userBchUtxos.find(u => u.satoshis >= BigInt(investAmountBch * 1e8) + 5000n);
    }
    if (!investmentUtxo) {
        throw new Error(`No funding UTXO found for ${address}`);
    }
    // a BCH-only utxo at index 1 to be used to balance the council output
    const councilUtxo = userBchUtxos.find(u => u.satoshis >= 1000n &&
